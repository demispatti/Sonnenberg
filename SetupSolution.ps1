...
Write-Output "Found following AssemblyInfo.cs files to check for strong signing:`n$AssemblyInfoFiles`nEnd AssemblyInfo.cs files list.`n"
...
Write-Output "DevCmd: $DevCmd`nSn.exe: $Sn`n`n"

# Assemblies that need a Public Key to be generated
[array]$PublicKeyFileNames = @("FileHelper", "ShellServer", "ServerManager", "Service", "ServiceManager", "StartService", "StopService", "ContextMenu")
Write-Output "Found following Assembiies that need to be signed publicly:`n$PublicKeyFileNames`nEnd display public Key file names.`n`n"

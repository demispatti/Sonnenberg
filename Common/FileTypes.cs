...
namespace Sonnenberg.Common
{
    ...
    public class FileTypes
    {
        ...
        {
            if (_fileExtensions.TextFileExtensions.Any(s => s.Equals(ext, StringComparison.OrdinalIgnoreCase)))
                return "text";

            if (0 == ext.Length) return "folder";

            return _fileExtensions.AllSupportedFileExtensions.Any(
                s => s.Equals(ext, StringComparison.OrdinalIgnoreCase))
                ? "supported"
                : "unsupported";
        }
